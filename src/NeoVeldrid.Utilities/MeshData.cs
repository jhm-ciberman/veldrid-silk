...
{
    /// <summary>
    /// An object describing generic mesh data. This can be used to construct a <see cref="DeviceBuffer"/> (vertex) and
    /// <see cref="DeviceBuffer"/> (index), and also exposes functionality for bounding box and sphere computation.
    /// </summary>
    public interface MeshData
    {
        /// <summary>
        /// Constructs a <see cref="DeviceBuffer"/> (vertex) from this <see cref="MeshData"/>.
        /// </summary>
        /// <param name="factory">The <see cref="ResourceFactory"/> to use for device resource creation.</param>
        /// <param name="cl">The <see cref="CommandList"/> to use for data upload.</param>
        /// <returns>A new vertex <see cref="DeviceBuffer"/>.</returns>
        DeviceBuffer CreateVertexBuffer(ResourceFactory factory, CommandList cl);

        /// <summary>
        /// Constructs a <see cref="DeviceBuffer"/> (index) from this <see cref="MeshData"/>.
        /// </summary>
        /// <param name="factory">The <see cref="ResourceFactory"/> to use for device resource creation.</param>
        /// <param name="cl">The <see cref="CommandList"/> to use for data upload.</param>
        /// <param name="indexCount">Receives the number of indices in the buffer.</param>
        /// <returns>A new index <see cref="DeviceBuffer"/>.</returns>
